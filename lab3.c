...
void* copy_file(void* arg) {
    ...
    int dest_fd;
    while(true) {
        dest_fd = open(data->dest, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
        if (dest_fd == -1) {
            if (errno == EMFILE) {
                sleep(3);
            }
        printf("Failed open dest_fd file\n");
        data_destructor(data);
        close(src_fd);
        return NULL;
        } else {
...
void* copy_dir(void* arg) {
    ...
    struct dirent* entry;
    while (((countThread < 1024) && (entry = readdir(dir)) != NULL)) {
